...
Supports:
  ...
  - ``parent_id``: exact match on parent issue (fetch children of a parent).
...
Top-level filtering (``parent_id IS NULL``) is handled in the service via the
``top_level`` flag, since fastapi_filter cannot express an IS NULL predicate on
an optional column.
"""
